...
const cacheGet = (uid) => state.userCache.get(uid);
...
const ICON_SRCS = { bell:"notifIcon", gear:"settingsIcon", paperclip:"paperclipIcon", pencil:"editPfpIcon", chat:"emptyIcon" };
for (const [k, id] of Object.entries(ICON_SRCS)) $el(id).src = ICONS[k];

const el = {
  authScreen: $el("authScreen"), appRoot: $el("appRoot"),
  tabLogin: $el("tabLogin"), tabSignup: $el("tabSignup"),
  loginForm: $el("loginForm"), signupForm: $el("signupForm"),
  loginEmail: $el("loginEmail"), loginPassword: $el("loginPassword"), loginBtn: $el("loginBtn"),
  signupUsername: $el("signupUsername"), signupEmail: $el("signupEmail"), signupPassword: $el("signupPassword"), signupBtn: $el("signupBtn"),
  authError: $el("authError"), myUsernameLabel: $el("myUsernameLabel"),
  backToLobbyBtn: $el("backToLobbyBtn"), notifBtn: $el("notifBtn"), notifBadge: $el("notifBadge"),
  notifDropdown: $el("notifDropdown"), notifList: $el("notifList"), markAllReadBtn: $el("markAllReadBtn"),
  settingsBtn: $el("settingsBtn"), profileBtn: $el("profileBtn"), myPfpBtn: $el("myPfpBtn"),
  statusDot: $el("statusDot"), statusText: $el("statusText"),
  lobbyView: $el("lobbyView"), roomView: $el("roomView"), roomGrid: $el("roomGrid"),
  openCreateRoomBtn: $el("openCreateRoomBtn"),
  onlineCount: $el("onlineCount"), capacityLabel: $el("capacityLabel"), userList: $el("userList"),
  adminPanel: $el("adminPanel"), adminPanelTitle: $el("adminPanelTitle"),
  kickPanelBtn: $el("kickPanelBtn"), roomSettingsBtn: $el("roomSettingsBtn"),
  chatHeadTitle: $el("chatHeadTitle"), chatHeadLock: $el("chatHeadLock"), chatHeadPublic: $el("chatHeadPublic"),
  adminBadge: $el("adminBadge"), closeDmBtn: $el("closeDmBtn"),
  chatContainer: $el("chatContainer"), emptyState: $el("emptyState"),
  messageInput: $el("messageInput"), fileInput: $el("fileInput"), fileLabel: $el("fileLabel"),
  sendBtn: $el("sendBtn"), toastEl: $el("toast"),
  replyBar: $el("replyBar"), replyBarName: $el("replyBarName"), replyBarPreview: $el("replyBarPreview"), replyBarCancel: $el("replyBarCancel"),
  filePreview: $el("filePreview"), filePreviewImg: $el("filePreviewImg"), filePreviewVideo: $el("filePreviewVideo"),
  filePreviewName: $el("filePreviewName"), filePreviewRemove: $el("filePreviewRemove"),
  userModal: $el("userModal"), modalPfp: $el("modalPfp"), modalName: $el("modalName"), modalBio: $el("modalBio"),
  modalDmBtn: $el("modalDmBtn"), modalBlockBtn: $el("modalBlockBtn"), modalKickBtn: $el("modalKickBtn"), modalCloseBtn: $el("modalCloseBtn"),
  profileModal: $el("profileModal"), myProfileAvatar: $el("myProfileAvatar"),
  profileHeroName: $el("profileHeroName"), profileHeroEmail: $el("profileHeroEmail"),
  editUsername: $el("editUsername"), editBio: $el("editBio"), editPfp: $el("editPfp"),
  saveProfileBtn: $el("saveProfileBtn"), cancelProfileBtn: $el("cancelProfileBtn"), profileError: $el("profileError"),
  createRoomModal: $el("createRoomModal"), createRoomCodeInput: $el("createRoomCodeInput"),
  createRoomName: $el("createRoomName"), createRoomMax: $el("createRoomMax"), createRoomPassword: $el("createRoomPassword"),
  createRoomBtn: $el("createRoomBtn"), cancelCreateRoomBtn: $el("cancelCreateRoomBtn"), createRoomError: $el("createRoomError"),
  passwordModal: $el("passwordModal"), passwordRoomCode: $el("passwordRoomCode"), joinRoomPassword: $el("joinRoomPassword"),
  submitPasswordBtn: $el("submitPasswordBtn"), cancelPasswordBtn: $el("cancelPasswordBtn"), passwordError: $el("passwordError"),
  kickedModal: $el("kickedModal"), kickedList: $el("kickedList"), closeKickedBtn: $el("closeKickedBtn"),
  roomSettingsModal: $el("roomSettingsModal"), settingsRoomName: $el("settingsRoomName"),
  settingsRoomMax: $el("settingsRoomMax"), settingsRoomPassword: $el("settingsRoomPassword"),
  saveRoomSettingsBtn: $el("saveRoomSettingsBtn"), cancelRoomSettingsBtn: $el("cancelRoomSettingsBtn"),
  deleteRoomBtn: $el("deleteRoomBtn"), roomSettingsError: $el("roomSettingsError"),
  settingsModal: $el("settingsModal"), themeSelect: $el("themeSelect"), languageSelect: $el("languageSelect"),
  currentPassword: $el("currentPassword"), newPassword: $el("newPassword"),
  changePasswordBtn: $el("changePasswordBtn"), logoutBtn2: $el("logoutBtn2"),
  closeSettingsBtn: $el("closeSettingsBtn"), settingsError: $el("settingsError")
};
...
onAuthStateChanged(auth, async (user) => {
  ...
  state.me = { uid: user.uid, email: user.email, username: d.username || "user", pfp: d.pfp || "", bio: d.bio || "" };
...
async function enterRoom(code, roomMeta) {
  ...
  const handler = (snapshot) => {
    ...
    renderMessage(snapshot.key, msg, msg.uid === state.me.uid);
    hideEmpty();
  };
  ...
  await set(state.presenceRef, { username: state.me.username, pfp: state.me.pfp, joinedAt: serverTimestamp() });
  ...
  state.kickedRef = ref(db, `rooms/${code}/kicked/${state.me.uid}`);
  state.kickedOff = onValue(state.kickedRef, async (snap) => {
...
async function detachFromRoom() {
  ...
  state.kickedRef = null; state.dmUid = null; state.dmQueryRef = null;
  updateAdminUI();
}
...
function renderNotifications() {
  ...
  el.notifList.onclick = async (e) => {
    const item = e.target.closest(".notif-item");
    if (!item) return;
    const idx = Array.from(el.notifList.children).indexOf(item);
    ...
    if ((n.type === "mention" || n.type === "reply") && n.roomCode) {
      if (state.roomCode !== n.roomCode) await requestJoinRoom(n.roomCode);
...
async function pushNotification(targetUid, payload) {
  if (targetUid === state.me.uid) return;
...
async function renderMessage(msgId, msg, isOwn) {
  ...
  const sameUser = state.lastGroupUid === msg.uid;
  const withinWindow = (now - state.lastGroupTime) < GROUP_MS;
  const inDmView = !!state.dmUid;
  ...
  const mentionedMe = plainText && state.me && extractMentions(plainText).includes(state.me.username.toLowerCase());

  if (!state.lastGroupEl || !sameUser || !withinWindow) {
    ...
    head.innerHTML = `<img class="mini-pfp" src="${sender.pfp || defaultPfp(sender.username)}" alt="">
      ${esc(sender.username)}<span class="group-time">${fmtTime(msg.timestamp)}</span>`;
    const body = document.createElement("div");
    body.className = "group-body";
    group.appendChild(head); group.appendChild(body);
    el.chatContainer.appendChild(group);
    state.lastGroupEl = group;
    state.lastGroupUid = msg.uid;
  }
  state.lastGroupTime = now;
  const body = state.lastGroupEl.querySelector(".group-body");
    ...
    if (src?.startsWith("data:")) {
      if (msg.mediaType === "image" || msg.mediaType === "gif") {
        ...
        if (msg.mediaType === "gif") {
          ...
          t.className = "gif-tag"; t.textContent = "GIF";
          line.appendChild(t);
        }
  ...
  replyBtn.addEventListener("click", () => {
    setReply({ uid: msg.uid, username: sender.username, previewText: plainText ? plainText.slice(0, 80) : "[media]", msgId });
    el.messageInput.focus();
  });
  actions.appendChild(replyBtn);
  if (isAdmin() && !inDmView && state.roomCode) {
    ...
    del.addEventListener("click", async () => {
      ...
      try {
        ...
        line.remove();
        if (!body.children.length) state.lastGroupEl.remove();
  ...
  if (!isOwn && inDmView && state.dmUid === msg.uid) {
    const k = `${state.dmUid}::${msgId}`;
    if (!dmNotified.has(k)) {
      dmNotified.add(k);
      pushNotification(msg.uid, {
        type: "dm", title: t("notifDm") + " " + sender.username,
        body: plainText ? plainText.slice(0, 80) : "[media]", fromUid: msg.uid
      });
    }
  }
}
...
async function renderUserList() {
  const uids = Object.keys(state.presenceData);
  el.onlineCount.textContent = uids.length;
...
async function openDm(otherUid) {
  if (!state.roomCode) return showToast("Join a room first");
  if (state.dmOff) { state.dmOff(); state.dmOff = null; }
  ...
  el.chatHeadTitle.textContent = "DM with " + other.username;
  el.closeDmBtn.classList.remove("hidden");
  el.adminBadge.classList.add("hidden");
  el.chatHeadLock.classList.add("hidden");
  el.chatHeadPublic.classList.add("hidden");
  const handler = (snapshot) => {
    const msg = snapshot.val();
    if (!msg) return;
    renderMessage(snapshot.key, msg, msg.uid === state.me.uid);
...
function exitDmView() {
  ...
  if (state.dmOff) { state.dmOff(); state.dmOff = null; }
  el.closeDmBtn.classList.add("hidden");
  if (state.roomCode) {
    el.chatHeadTitle.textContent = "# " + (state.roomMeta?.name || state.roomCode);
    el.chatHeadLock.classList.toggle("hidden", !state.roomMeta?.hasPassword);
    el.chatHeadPublic.classList.toggle("hidden", !state.roomMeta?.isPublic && state.roomCode !== PUBLIC_ROOM);
    updateAdminUI();
  }
}

el.closeDmBtn.addEventListener("click", async () => {
  exitDmView();
  resetChatUI();
...
async function sendMessage() {
  ...
  if (state.dmUid) {
    ...
    await push(ref(db, dmPath(state.me.uid, state.dmUid)), {
      ...
      mediaData: hasMedia ? encryptText(state.pendingFile.dataUrl, encKey) : null,
...
el.fileInput.addEventListener("change", (e) => {
  ...
  if (!file) return;
  if (file.size > 1.5 * 1024 * 1024) { showToast("File too big (1.5MB max)"); el.fileInput.value = ""; return; }
  let type = null;
  if (file.type === "image/gif") type = "gif";
  else if (file.type.startsWith("image/")) type = "image";
  else if (file.type.startsWith("video/")) type = "video";
  else { showToast("Images, GIFs, videos only"); el.fileInput.value = ""; return; }
  const objectUrl = URL.createObjectURL(file);
  const r = new FileReader();
  r.onload = (ev) => {
    if (state.pendingFile?.objectUrl) URL.revokeObjectURL(state.pendingFile.objectUrl);
    state.pendingFile = { type, dataUrl: ev.target.result, objectUrl, name: file.name };
    el.filePreviewName.textContent = `${type.toUpperCase()} · ${file.name}`;
    if (type === "video") {
      el.filePreviewImg.classList.add("hidden");
      el.filePreviewVideo.classList.remove("hidden");
      el.filePreviewVideo.src = objectUrl;
    } else {
      el.filePreviewVideo.classList.add("hidden");
      el.filePreviewImg.classList.remove("hidden");
      el.filePreviewImg.src = objectUrl;
    }
